...
export function Dashboard({ onLogout }: { onLogout: () => void }) {
    ...
    const { data: bandwidth } = useQuery({
        queryKey: ['bandwidth'],
        queryFn: () => Promise.resolve(null as BandwidthStats | null),
    ...
    const handleDropOnFolder = async (e: React.DragEvent, targetFolderId: number | null) => {
        ...
        if (rawIds) {
            try { idsToMove = JSON.parse(rawIds); } catch {}
        }
                ...
                {showMoveModal && (
                    <MoveToFolderModal
                        ...
                        onSelect={async (targetFolderId: number | null) => {
                            if (moveFileTarget) {
                                try {
                                    await api.renameFile(moveFileTarget.id, moveFileTarget.name, targetFolderId);
                                    queryClient.invalidateQueries({ queryKey: ['files', activeFolderId] });
